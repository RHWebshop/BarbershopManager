...
Barbershop Manager is a full-stack platform built for managing appointments and store functionality.
...
- `apps/barbershop-manager-client`: A React application built with Vite and Shadcn UI.
- `apps/barbershop-manager-api`: A NestJS backend API.

### Packages

- `packages/barbershop-manager-schemas`: Shared Zod validation schemas (e.g. authentication, products).
- `packages/barbershop-manager-types`: Shared TypeScript interfaces and types (e.g. Users, Appointments).
...
2. **Build Packages**
   ...
   _(Alternatively, to specifically build a package: `pnpm -F @barbershop-manager/schemas build`)_
...
3. **Running the Applications**
   ...
   **Alternatively, to start them individually:**
   ...
   ```bash
   pnpm -F barbershop-manager-client dev
   ```

   Backend API application (NestJS dev server):

   ```bash
   pnpm -F barbershop-manager-api start:dev
   ```
