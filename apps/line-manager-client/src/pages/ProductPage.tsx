...
export function ProductPage() {
  ...
  const handleIncrement = () => product && updateQuantity(product.id, quantity + 1);
  const handleDecrement = () => product && updateQuantity(product.id, quantity - 1);

  return (
    <div className="space-y-10">
      <div className="space-y-2">
        <h1 className="text-4xl font-extrabold tracking-tight">{product ? product.name : "דף מוצר"}</h1>
        <p className="text-lg text-muted-foreground">
          {product ? "כאן תוכלו לראות פרטים על המוצר ולהוסיף אותו לסל." : "המוצר המבוקש לא נמצא."}
        </p>
      </div>
      ...
      {false && (
        <div className="space-y-10">
          ...
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-muted-foreground">
            <Link to="/store" className="hover:text-primary hover:underline">חנות</Link>
            ...
            <div className="flex flex-col justify-center space-y-8">
              <div className="space-y-4">
                ...
                <div className="flex items-baseline gap-4">
                  ...
                  {product.price > 100 && (
                    <span className="text-sm font-medium text-green-600">משלוח חינם</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6 pt-4">
                  ...
                  {quantity === 0 ? (
                    ...
                    <div className="flex flex-1 items-center gap-2 overflow-hidden rounded-xl border-2 border-primary/20 bg-muted/30 p-1">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="min-w-[3rem] text-center text-xl font-black">
                          {quantity}
                        </span>
                        <Button
                          variant="ghost"
                ...
                {quantity > 0 && (
                  <div className="flex items-center gap-2 rounded-lg bg-primary/5 p-4 text-primary">
                    ...
                    <Link to="/cart" className="ms-auto text-sm font-bold underline transition-opacity hover:opacity-80">
                      מעבר לסל
                    </Link>
                  </div>
                )}
