---
description: UI engineering rules (React + Vite) for LineManager with Tailwind, shadcn/ui, Zod, react-hook-form, TanStack Query
globs: ["apps/line-manager-admin/**/*.{ts,tsx}", "apps/line-manager-client/**/*.{ts,tsx}"]
alwaysApply: true
---

# UI Rules: LineManager (React + Vite)

These rules apply to `apps/line-manager-admin` and `apps/line-manager-client`.
...
- For request/response validation:
  - Prefer using shared Zod schemas from `packages/line-manager-schemas` when available.
- Types:
  - Use shared types from `packages/line-manager-types` only for shared contracts.
